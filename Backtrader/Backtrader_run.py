...
todate = datetime.date(2018, 1, 8)  # Last date to trade
invested = 1000  # How much to invest in each stock
start_date = datetime.datetime(2016, 9, 7, 9, 30, 00)  # Start  date
end_date = datetime.datetime(2018, 1, 8, 16, 00, 00)  # Ending date
...
def strategy_one():
    ...
    for line in my_pair_file: 
        ...
        my_result_file.write("Pair: " + stock1 + " " + stock2 + "\n")  # To know which trades below belong to this pair
        cerebro.addstrategy(Strategy_pairGen, stock1=stock1, stock2=stock2, distance=0.5, period=200, invested=invested,
    ...
    # Calculate the profit for the total portfolio (all pairs)
    total_portfolio_value = sum(endValueForEachPair)-len(endValueForEachPair)*portfolio_value_input
...
def strategy_two():
    ...
    # Calculating portfolio value for all stocks so far

...
class Strategy_pairGen(Strategy):
    ...
    def __init__(self):
        Strategy.__init__(self, self.params.invested, self.params.period,
                          self.params.todate,
                          self.params.my_result_file)  # Initiate the general general parameters for strategies
    ...
    def next(self):
        ...
        if len(self.stock1Data) >= self.period:  # To check if we can start making trades
            ...
            z_score = self.linearRegression(relevant_data_stock1, relevant_data_stock2,
                                            self.period)  # Want to calculate the z_score

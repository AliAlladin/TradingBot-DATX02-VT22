...
class Strategy(bt.Strategy):
    ...
    def __init__(self, invested, period, todate, my_result_file):
        ...
        self.datap = os.path.join(modpath,
                                  'Results/plotvalues.txt')  # The data of pairs comes from Pairs.txt which we read
...
class Strategy_pairGen(Strategy):
    ...
    def __init__(self):
        ...
        Strategy.__init__(self, self.params.invested, self.params.period,
                          self.params.todate,
                          self.params.my_result_file)  # Initiate the general parameters for any of the strategies
    ...
    def next(self):
        ...
        if self.firstTime:
            self.oldDate = str(self.datas[0].datetime.date(
                0))  # We need the oldDate to be equal to the first day which cannot be initilized in init
